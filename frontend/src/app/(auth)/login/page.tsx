...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.error) {
        response = await fetch("http://localhost:8080/api/auth/admin/login", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      <div className="w-full max-w-md rounded-lg bg-[#1a1a1a] p-8 shadow-2xl border border-gray-800">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-[#d4af37]">
            Login to Amanda Cinema
          </h1>
          <p className="text-[#f5f5f5]/60 mt-2">Welcome back!</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-[#f5f5f5]/60">
          ...
          <Link href="/register" className="text-[#d4af37] hover:text-[#f5f5f5] font-medium">
            Sign up
          </Link>
        </p>
      </div>
